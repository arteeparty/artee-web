import LovenseBase from "./LovenseBase";
...
export default class LovenseWebBluetooth extends LovenseBase {
  static findDevice() {
  ...
  open() {
    return this._device.gatt
      .connect()
      .then(server => {
        return server.getPrimaryService(HUSH_SERVICE);
      })
      .catch(er => {
        console.log(er);
      })
      ...
      .then(char => {
        ...
        return this._rx.startNotifications().then(_ => {
          this._rx.addEventListener("characteristicvaluechanged", e =>
            console.log(new TextDecoder("ASCII").decode(e.target.value))
          );
        });
      });
  }
  ...
  _writeFromQueue() {
    ...
    this._write(this._msg_queue[0]).then(() => {
      this._msg_queue.shift();
